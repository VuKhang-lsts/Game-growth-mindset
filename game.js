...
class Dog {
  ...
  draw() {
  // --- vẽ chó (Mr. Gold) có xoay nhẹ theo vận tốc ---
  const angle = Math.max(-0.6, Math.min(0.6, this.vy / 12));
  const W = 110, H = 62;  // kích thước hiển thị chó (tuỳ chỉnh)

  ctx.save();
  ctx.translate(this.x, this.y);
  ctx.rotate(angle);
  if (spriteReady) {
    ctx.drawImage(spriteMrGold, -W/2, -H/2, W, H); // vẽ ảnh chó
  } else {
    ctx.fillStyle = "#f4b400";
    ctx.beginPath(); ctx.arc(0, 0, 18, 0, Math.PI*2); ctx.fill();
  }
  ctx.restore(); // trả về hệ toạ độ gốc (không xoay)

  // --- vẽ Exciter cố định ở phía trên: KHÔNG xoay, KHÔNG theo Y của chó ---
  if (EXCITER_SHOW && exciterReady) {
    const ratio = (exciterImg.naturalHeight || 1) / (exciterImg.naturalWidth || 1);
    const eW = W * EXCITER_TOP_SCALE * 1.6;
    const eH = eW * ratio;

    const drawX = this.x - eW / 2 + EXCITER_TOP_OFFSET_X; // “đi theo” X của chó
    const drawY = EXCITER_TOP_Y - eH / 2;                  // Y cố định trên cùng

    ctx.save();
    ctx.globalAlpha = EXCITER_ALPHA;
    ctx.drawImage(exciterImg, drawX, drawY, eW, eH);       // không rotate
    ctx.globalAlpha = 1.0;
    ctx.restore();
  }
}
...
function loseLife(){
  if (lives > 1){
    ...
    lives = 0; updateLivesHUD(); gameOver();
  }
}
...
function gameOver(){
  ...
  bestEl.textContent = `Best: ${best}`;
  msgEl.textContent = "Mr.Gold đã bị anh Ex tóm😅 — Nhấn Space / Click để chơi lại";
  hideQBanner(); setTimerText("");
}
